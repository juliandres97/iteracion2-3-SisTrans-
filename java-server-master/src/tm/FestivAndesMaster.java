...
public class FestivAndesMaster {
	...
	/**
	 * Metodo que modela la transaccion que modifica un miembro del Compania a
	 * la base de datos. <b> post: </b> se ha modificado el miebro que entra
	 * como parametro
	 * 
	 * @param compania
	 *            - el miembro a modificar. miembro != null
	 ...
	 *             - cualquier error que se genere durante la transacci贸n
	 */
	public Compania searchCompaniaByNitPassword(String Nit, String password) throws Exception {
	...
	/**
	 * Metodo que modela la transaccion que agrega un espectaculo del
	 * Espectaculo a la base de datos. <b> post: </b> se ha agregado el miebro
	...
	public void addEspectaculo(Espectaculo espectaculo) throws Exception {
		...
		try {
			...
			if (espectaculo.getCompanias().isEmpty() || espectaculo.getCompanias() == null
			...
			try {
				...
				daoCompaniaEspectaculo.cerrarRecursos();
		...
		try {
			...
			if (sitio.getEspecificaciones() != null && !sitio.getEspecificaciones().isEmpty())
				for (Especificacion especificacion : sitio.getEspecificaciones())
					daoSitioEspec.addEspecificacionToSitio(especificacion.getId(), sitio.getId(),
							especificacion.getEspecificacion());
			...
			if (sitio.getLocalidades().isEmpty() || sitio.getLocalidades() == null
			...
			try {
				...
				daoSitioLocalidad.cerrarRecursos();
				daoSitioEspec.cerrarRecursos();
				if (this.conn != null)
					this.conn.close();
			} catch (SQLException exception) {
				System.err.println("SQLException closing resources:" + exception.getMessage());
...
//	private void addFuncionPriv(Funcion funcion) throws Exception {
...
//	}
	...
	public void addGeneros(Cliente cliente, GeneroList generos) throws Exception {
		...
		try {
			...
			if (generos.getGeneros().isEmpty() || generos.getGeneros() == null
	...
	public void comprarBoletaReg(Cliente cliente, Boleta boleta) throws Exception {
		...
		DAOTablaCupos daoCupos = new DAOTablaCupos();
		DAOTablaEspectador daoEspectador = new DAOTablaEspectador();
...
//	private void comprarBoletasAbonoReg(Cliente cliente, List<Boleta> boletas) throws Exception {
//		DAOTablaBoletas daoBoletas = new DAOTablaBoletas();
//		DAOTablaVentas daoVentas = new DAOTablaVentas();
//		DAOTablaCupos daoCupos = new DAOTablaCupos();
//
...
//			conn.commit();
//
////			if (boletas.size() == cantBoletasCompradas) {
////				conn.commit();
////			} else {
////				conn.rollback();
////				throw new Exception("No hay cupo disponible para la funci髇.");
...
//	}
	...
	public void realizarAbono(Cliente cliente, Abono abono) throws Exception {
		...
		try {
			...
			for (Boleta boleta : abono.getBoletas()) {
				if (daoCupos.getCupoActual(boleta.getIdEspectaculo(), boleta.getIdFuncion()) > 0) {
					
					daoBoletas.addBoletaAbono(abono, boleta);
					daoVentas.addVenta(cliente, boleta);
					daoCupos.updateCupoVentaBoleta(boleta);
			...
			if (cantBoletasCompradas == abono.getBoletas().size()) {
				conn.commit();			
			} else {
				conn.rollback();
...
//	private void regresarBoletasAbonoReg(Cliente cliente, List<Boleta> boletas) throws SQLException, Exception {
//		DAOTablaBoletas daoBoletas = new DAOTablaBoletas();
//		DAOTablaVentas daoVentas = new DAOTablaVentas();
//		DAOTablaCupos daoCupos = new DAOTablaCupos();
//
...
//	}
	...
	public void regresarAbono(Cliente cliente, Abono abono) throws Exception {
		...
		try {
			...
			List<Boleta> boletasParaRegresar = daoBoletas.buscarBoletasAbono(abono);
			
			if (boletasParaRegresar.isEmpty()) {
				conn.rollback();
				throw new Exception("No hay boletas asociadas al abono.");
			} else {				
				for (Boleta boleta : boletasParaRegresar) {
					daoVentas.regresarVenta(cliente, boleta);
					daoCupos.updateCupoRegresoBoleta(boleta);
				}
				
				daoBoletas.regresarBoletasAbono(abono);
	...
	public void asistentesFuncionPorCompania(String nombreComp, String fechaInicial, String fechaFinal) throws SQLException, Exception {
		...
		try {
			...
			Compania compania = daoCompanias.searchCompaniaByNombre(nombreComp);
			List<Espectaculo> espectaculos = daoEspectaculos.searchEspectaculosDeCompania(compania.getId());
